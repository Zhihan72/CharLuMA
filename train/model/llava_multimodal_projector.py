...
class MLP_LuMA(nn.Module):
    ...
    def __init__(
        ...
        for lin in self.lang_Wq.values():
    ...
    def _router_logits(self, h_bar: torch.Tensor, lang_ids: torch.LongTensor) -> torch.Tensor:
        n_langs = len(torch.unique(lang_ids))
        print('n_langs: {}'.format(n_langs))
        outs = []
        for i, lid in enumerate(lang_ids.tolist()):
            lang = self.languages[lid]
            outs.append(self.lang_Wq[lang](h_bar[i:i+1]))
